...
stat_t nexti(y64sim_t *sim)
{
    ...
    switch (icode) {
      ...
      case I_RRMOVQ:  /* 2:x regA:regB */
        ...
        ++next_pc;
        sim->pc = next_pc;
        if(!cond_doit(sim->cc,ifun)) break;
        regid_t rA,rB;
        rA = GET_REGA(reg);
        rB = GET_REGB(reg);
      ...
      case I_CALL: /* 8:x imm */
        ...
        if(set_long_val(sim->m,valE,valP) == FALSE){
            //                 printf("***valB = %lx \n",valB);
      ...
      case I_PUSHQ: /* A:0 regA:F */
        {
        byte_t reg = 0;
        if(get_byte_val(sim->m, next_pc, &reg) == FALSE){
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        regid_t rA;
        ...
        long_t valE,valP,test;
        valP = ++next_pc;
        valE = valB - 8;
        set_reg_val(sim->r,REG_RSP,valE);
        if(set_long_val(sim->m,valE,valA) == FALSE){
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc,valE);
            return STAT_ADR;
        }
        sim->pc = valP;
